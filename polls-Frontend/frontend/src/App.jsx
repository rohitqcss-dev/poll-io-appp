...
function App() {
  ...
  const handleVote = async (id, optionIndex) => {
    if (votedPolls.some(v => v.startsWith(id))) {
      ...
      <div className="max-w-3xl mx-auto space-y-10">
        ...
        <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm">
          <form onSubmit={createPoll} className="space-y-5">
            <input type="text" placeholder="What's the question?" value={question} onChange={(e) => setQuestion(e.target.value)} className="w-full p-4 bg-slate-50 border-none rounded-2xl font-bold focus:ring-2 focus:ring-blue-500 outline-none" required />
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <input type="text" placeholder="Option 1" value={option1} onChange={(e) => setOption1(e.target.value)} className="p-4 bg-slate-50 rounded-2xl outline-none font-medium" required />
              <input type="text" placeholder="Option 2" value={option2} onChange={(e) => setOption2(e.target.value)} className="p-4 bg-slate-50 rounded-2xl outline-none font-medium" required />
              <input type="text" placeholder="Option 3 " value={option3} onChange={(e) => setOption3(e.target.value)} className="p-4 bg-slate-50 rounded-2xl outline-none font-medium" />
            ...
            <button type="submit" className="w-full bg-blue-600 text-white p-4 rounded-2xl font-black uppercase tracking-widest text-sm hover:bg-slate-900 transition-all shadow-lg shadow-blue-100">Create Poll</button>
          </form>
        </div>


        <div className="flex bg-slate-200/50 p-1 rounded-2xl w-fit mx-auto border border-slate-200">
          {['all', 'active', 'expired'].map(t => (
            <button key={t} onClick={() => setFilter(t)} className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${filter === t ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400'}`}>{t}</button>
          ))}
        </div>
        ...
        <div className="space-y-6 pb-20">
          {polls
            ...
            .map((item) => {
              ...
              return (
                <div key={item._id} className={`bg-white p-8 rounded-[2.5rem] border-2 transition-all ${isExpired ? 'border-slate-100 opacity-90' : 'border-white shadow-xl shadow-slate-200/50'}`}>
                  ...
                  <div className="space-y-3">
                    {item.options.map((opt, i) => {
                      ...
                      return (
                        <button key={i} disabled={isExpired || hasVoted} onClick={() => handleVote(item._id, i)}
                          className={`w-full flex justify-between items-center p-5 rounded-2xl border-2 transition-all duration-300
                            ...
                            ${!isExpired && !hasVoted && 'hover:border-blue-200 hover:bg-white'}
                          `}>
                          <div className="flex items-center gap-3">
                            ...
                            {isWinningOption && <span className="text-[8px] bg-amber-500 text-white px-2 py-0.5 rounded-full uppercase font-black tracking-tighter italic">Winner</span>}
                          </div>
                          <span className={`font-black text-xs ${isWinningOption ? 'text-amber-600' : (isMyVote ? 'text-blue-600' : 'text-slate-400')}`}>{opt.votes || 0}</span>
                        </button>
                      )
                    })}
                  </div>
            ...
            {polls.length === 0 && <p className="text-center text-slate-400 font-bold py-10 uppercase tracking-widest text-xs border-2 border-dashed rounded-[2rem]">No polls found</p>}
        </div>
      </div>
    </div>
  )
